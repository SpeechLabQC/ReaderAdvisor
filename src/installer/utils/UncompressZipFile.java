...
public class UncompressZipFile {
    ...
    private List<String> extensionSkipFilter = new LinkedList<String>();
    ...
    public void addExtensionSkipFilter(String extension){
        extensionSkipFilter.add(extension);
    }
    ...
    public void init() throws IOException {
        if(runOnce){
            ZipInputStream _zin = new ZipInputStream(temp);
            ZipEntry entry;
            // Store all the files
            while ((entry = _zin.getNextEntry()) != null) {
                // Do not include files that we need to filter
                if(skipFile(entry.getName())) continue;
                // Add filename to the list
                zipEntryList.add(entry.getName());
            }
            _zin.close();
            // Disable run once flag
            runOnce = false;
        }
    }

    // Return true if the file ends with an extension that needs to be filtered. Return false otherwise.
    private boolean skipFile(String fileName){
        for(String ext : extensionSkipFilter){
