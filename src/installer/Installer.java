    ...
    */
    ...
    private void installingSoftware(){
        ...
        try{
            ...
            // Only skip files when installing the User edition
            if(currentEdition.equals(EDITION.USER)){
                // Skip batch files if the OS is not Windows
                if(!InstallerUtils.isWindows()){
                    filesToSkip.add(".bat");
                    filesToSkip.add(".exe");
                    filesToSkip.add(".dll");
                } else{// Skip shell files if the OS is Windows
                    filesToSkip.add(".sh");
                    filesToSkip.add(".command");
                    filesToSkip.add(".a");
                    filesToSkip.add(".la");
                    filesToSkip.add(".lai");
                    filesToSkip.add(".so");
                    filesToSkip.add(".so.0");
                    filesToSkip.add(".so.0.0.0");
                    filesToSkip.add(".so.1");
                    filesToSkip.add(".so.1.1.1");
                }
            }
            // Create the worker thread that will perform the installation
            installationWorker = new InstallationWorker(installationDirectory,editionToInstall,textArea,filesToSkip);
    ...
    // TODO: Impletement an alternative approach that will create the installation in the temp directory and after it has been created.
    // It will move this file into the installation directory. In this way, the installation won't need to be deleted.
    /*
