...
public class GrammarUtils {
    ...
    public static synchronized boolean createGrammar(String text){
        return createGrammarInASingleLine(text);
        //return createGrammarWithOutProbability(text);
        //return createGrammarWithProbability(text);
    }

    public static synchronized boolean createGrammarInASingleLine(String text){
        boolean isGrammarCreatedSuccessfully = false;
        ...
        if(text != null){
            ...
            StringBuilder grammar = new StringBuilder("#JSGF V1.0" + EnvironmentUtils.NEW_LINE +
                    "grammar " + SpeechManager.GRAMMAR + ";" + EnvironmentUtils.NEW_LINE);
            grammar.append("public <all_line> =");
            StringBuilder list = new StringBuilder();
            int counter = 0;
            for(String line : text.split("\n")){
                line = line.trim();
                if(!line.isEmpty()){
                    list.append("<line_").append(++counter).append("> = ").append(line.trim()).append(";").append(EnvironmentUtils.NEW_LINE);
                    grammar.append(" <line_").append(counter).append(">");
                }
            }
            grammar.append(";").append(EnvironmentUtils.NEW_LINE);
            grammar.append(list.toString());
            try{
                ...
                if(!temp.exists()){
                    ...
                    else{ temp.deleteOnExit(); } // If this method creates this directory then ensure to delete it when exiting the JVM
                }
                // Create the document to store the Grammar
                ...
                bw.write(grammar.toString());
                bw.close();
                // The Grammar was created successfully
                isGrammarCreatedSuccessfully = true;
            }catch(Exception e){ e.printStackTrace(); }
        }
        return isGrammarCreatedSuccessfully;
    }
    ...
    public static synchronized boolean createGrammarWithOutProbability(String text){
        ...
        for(String line : text.split("\n")){
            line = line.trim();
            if(!line.isEmpty()){
                grammar += "public <line_" + (++counter) + "> = " + line.trim() + ";" + EnvironmentUtils.NEW_LINE + EnvironmentUtils.NEW_LINE;
            }
        }
