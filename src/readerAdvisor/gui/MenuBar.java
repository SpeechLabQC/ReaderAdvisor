                    ...
                    if (fileSummary.getFileData().toString() != null && !fileSummary.getFileData().toString().trim().isEmpty()) {
                        ...
                        if(SpeechManager.getInstance().getSpeechConfiguration() == null){
                            ...
                            String languageModel =
                                    SpeechManager.getInstance().setAndVerifyLanguageModel(
                                                                    GlobalProperties.getInstance().getProperty("sphinx.languageModel"));
                            // Set up the Grammar Configuration
                            SpeechManager.getInstance().setSpeechConfiguration(
                                    GlobalProperties.getInstance().getProperty(
                                            "speechManager.speechConfiguration." + languageModel, // Grammar Configuration
                                            SpeechManager.GRAMMAR_MODEL)); // Provide a default configuration if the above doesn't work
                        }
                        // The current Sphinx configuration is selected by the Sphinx Configuration Panel 'configuration' drop down
                        // Do not leave the user waiting until the grammar is created
