...
public class FileUtils {
     ...
     */
    public static synchronized String normalizeNewLine(String text){
    ...
    */
     ...
     * @param directory Directory to be created if it doesn't exists
     * @param deleteOnExist Ensure that the directory gets delete when the JVM exists
     */
    public static synchronized void ensureDirectoryExistsWithReadWriteAccess(String directory, boolean deleteOnExit) throws IOException, EnvironmentException {
            // Ensure that there exists a directory for the grammar files
            File folder = new File(directory);
            // If the folder does not exists then create it
            if(!folder.exists()){
                if(!folder.mkdir()){
                    throw new IOException("[" + directory + "] directory cannot be created!");
                }
            }
            ...
            if(deleteOnExit) { folder.deleteOnExit(); }
    }
